...
extern "C" word* psp_sdl_get_blit_addr();
...
// GetColor
// ----------------------------------------------------------------------------
extern "C" word atari_pal16[256];

static inline word maria_GetColor(byte data) {
  if(data & 3) {
    return atari_pal16[memory_ram[BACKGRND + data]];
  }
  else {
    return atari_pal16[memory_ram[BACKGRND]];
  }
}
...
// ----------------------------------------------------------------------------
void maria_Reset( ) {
  if (! maria_surface) maria_surface = psp_sdl_get_blit_addr();
  maria_scanline = 1;
